...
<div class="max-w-7xl mx-auto space-y-6 px-2 sm:px-4">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-black text-amber-400">Pengaturan Kurs Mata Uang</h1>
            <p class="text-xs text-gray-400">Perbarui nilai beli, jual, dan bendera secara langsung.</p>
        </div>
        
        <!-- Tombol Toggle Form Tambah -->
        <button onclick="toggleAddForm()" class="w-full sm:w-auto justify-center bg-amber-400 hover:bg-amber-300 text-[#0A1245] font-black px-5 py-2.5 rounded-xl shadow-lg transition flex items-center gap-2 text-sm">
            <span>➕</span> Tambah Mata Uang
        </button>
    </div>

    <!-- FORM TAMBAH MATA UANG BARU -->
    <div id="addCurrencyForm" class="hidden bg-[#0A1245] border-2 border-amber-400/50 rounded-2xl p-4 sm:p-6 shadow-2xl transition-all">
        <h2 class="text-lg font-black text-amber-400 mb-4 flex items-center gap-2">
            <span>✨</span> Tambah Mata Uang Baru
        </h2>
        <form action="{{ route('admin.currencies.store') }}" method="POST" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="number" step="any" name="buy_rate" placeholder="15000" required class="w-full bg-[#050B2E] border border-slate-700 text-green-400 font-mono font-bold rounded-xl p-2.5 focus:ring-2 focus:ring-amber-400 focus:outline-none">
            </div>
            <div>
                <label class="block text-xs font-bold text-gray-300 mb-1">We Sell (Jual)</label>
                <input type="number" step="any" name="sell_rate" placeholder="15200" required class="w-full bg-[#050B2E] border border-slate-700 text-amber-400 font-mono font-bold rounded-xl p-2.5 focus:ring-2 focus:ring-amber-400 focus:outline-none">
            </div>
            <div>
            ...
            <div class="flex gap-2 sm:col-span-2 lg:col-span-1">
                ...
                <button type="button" onclick="toggleAddForm()" class="bg-slate-700 hover:bg-slate-600 text-gray-300 font-bold px-4 py-2.5 rounded-xl shadow transition">
                    Batal
                </button>
            </div>
        </form>
    ...
    <form action="{{ route('admin.currencies.updateAll') }}" method="POST">
        ...
        <!-- Container overflow-x-auto untuk membuat tabel responsive di HP -->
        <div class="bg-[#0A1245] border border-amber-500/30 rounded-2xl shadow-2xl overflow-x-auto mb-6">
            <table class="w-full text-left min-w-[700px]">
                <thead>
                    <tr class="bg-amber-400 text-[#0A1245] text-xs sm:text-sm font-black uppercase">
                        <th class="py-3 px-4 text-center w-16">Flag</th>
                        <th class="py-3 px-4 w-24">Valas</th>
                        <th class="py-3 px-4 min-w-[150px]">Nama Mata Uang</th>
                        <th class="py-3 px-4 min-w-[140px]">We Buy (Beli)</th>
                        <th class="py-3 px-4 min-w-[140px]">We Sell (Jual)</th>
                ...
                <tbody class="divide-y divide-slate-800 text-sm bg-[#0A1245]">
                    ...
                    <tr class="hover:bg-slate-900/50 transition-colors">
                        ...
                        <td class="py-3 px-4 text-center">
                            @if($currency->flag)
                                <img src="https://flagcdn.com/w80/{{ strtolower($currency->flag) }}.png" class="w-7 h-auto rounded inline-block border border-amber-400/30" alt="{{ $currency->code }}">
                            @else
